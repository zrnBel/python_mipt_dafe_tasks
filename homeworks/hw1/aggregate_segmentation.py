...
def aggregate_segmentation(
    ...
    for data in segmentation_data:
        (audio_id, segment_id, segment_start, 
         segment_end, voice_type) = data.values()
        ...
        all_none = all(
            val is None 
            for val in (segment_start, segment_end, voice_type)
            )
        ...
        if not all((isinstance(segment_start,float), isinstance(segment_end,float),
    ...
    for audio_id, segments_data in temp_data.items():
        ...
        all_none = all(
            seg["type"] is None for seg in segments_data.values()
        )
